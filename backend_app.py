...
async def stats_reporter(color: str):
    ...
    Args:
        color (str): цвет линии на графике
    Корутина для сбора информации о использовании системных ресурсов компьютера нашем микросервисом.
    Далее информация отправляется на центральный сервер collector.
    """
    p = psutil.Process()
